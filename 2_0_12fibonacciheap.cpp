...
struct node {// Creating a structure to represent a node in the heap
...
node* mini = NULL;// Creating min pointer as "mini"

int no_of_nodes = 0;// Declare an integer for number of nodes in the heap
...
void Fibonnaci_link(node* ptr2, node* ptr1){// Linking the heap nodes in parent child relationship
...
void Consolidate(){// Consolidating the heap
    int temp1;
...
void Extract_min(){// Function to extract minimum node in the heap
    if (mini == NULL) cout << "The heap is empty" << endl;
    else {
        node* temp = mini;
        node* pntr;
        pntr = temp;
...
void Cut(node* found, node* temp){// Cutting a node in the heap to be placed in the root list
...
void Cascase_cut(node* temp){// Recursive cascade cutting function
    node* ptr5 = temp->parent;
    if (ptr5 != NULL) {
        if (temp->mark == 'W') temp->mark = 'B';
        else {
            Cut(temp, ptr5);
            Cascase_cut(ptr5);
        }
...
void Decrease_key(node* found, int val){// Function to decrease the value of a node in the heap
    if (mini == NULL) cout << "The Heap is Empty" << endl;
    ...
    if (found->key < mini->key) mini = found;
}

void Find(node* mini, int old_val, int val){// Function to find the given node
    node* found = NULL;
    node* temp5 = mini;
    temp5->c = 'Y';
    node* found_ptr = NULL;
...
void Deletion(int val){// Deleting a node from the heap
    if (mini == NULL) cout << "The heap is empty" << endl;
    else {
        Find(mini, val, 0);// Decreasing the value of the node to 0. Calling Extract_min function to
        Extract_min();// delete minimum value node, which is 0
...
void display(){// Function to display the heap
    node* ptr = mini;
    if (ptr == NULL) cout << "The Heap is Empty" << endl;
    else {
