#greatest common divisor: given two or more numbers, there could be two or more number that divide the given numbers. Among these numbers one that divides both of the given numbers and is the greatest of all is gcd.
...
def recursive_euclid_gcd(num1,num2):# d=gcd m=num1 n=num2 is divide both so m=ad and n=b*d==>  (m-n)=(ad - bd)==>   (m-n)=(a-b)d
    ...
    if(num1%num2)==0:#if num2 is the gcd
        ...
        diff=num1-num2
        return euclid_gcd(max(num2,diff),min(num2,diff))#diff>n? posssible!
...
#we can do better. For difference euclid gcd when smaller num2 is very small it longer. ex: euclid_gcd(101,2) loop 50 steps steps euclid_gcd(99,2) euclid_gcd(97,2)...euclid_gcd(2,1)
def remainder_euclid_gcd(num1,num2):#if n doesn't divide m. Then m=qn+r (q=quotient, r=remainder) then m=ad, n=bd. so ad=q(bd) + r. It follows that r=cd, so d divides r as well. 
    ...
    if(num1%num2)==0:#if num2 is the gcd
        ...
        return remainder_euclid_gcd(num2,num2%num1)
